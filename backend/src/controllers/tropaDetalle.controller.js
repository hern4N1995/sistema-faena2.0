...
const db = require('../db'); // tu cliente pg (db.query)
...
async function updateDetalle(req, res) {
  try {
    ...
    if (!detalleId) return err(res, 400, 'detalleId requerido');
    ...
    if (
      ...
      // normalizamos a entero si es posible
      const n = Number(id_cat_especie);
      updates.id_cat_especie = Number.isFinite(n) ? n : id_cat_especie;
    }
    ...
    // ahora traemos la fila con el nombre/descripcion de la categoría para que el frontend tenga label
    // (ajusta nombres de columnas/tablas según tu esquema)
    const joinSql = `
      SELECT td.*,
             ce.id_cat_especie AS categoria_id,
      ...
      LIMIT 1;
    `;
    const joined = await db.query(joinSql, [
      row.id_tropa_detalle || row.id || detalleId,
    ]);
